...
void* Interface::GetPtr(const char* filename, const char* interfaceSymbol)
{
    ...
    if (!handle) {
        console->DevWarning("chaos: Failed to open module %s!\n", filename);
        return nullptr;
    }
    ...
    if (!CreateInterface) {
        console->DevWarning("chaos: Failed to find symbol CreateInterface for %s!\n", filename);
        return nullptr;
    }
    ...
    for (auto& current = s_pInterfaceRegs; current; current = current->m_pNext) {
        if (std::strncmp(current->m_pName, interfaceSymbol, std::strlen(interfaceSymbol)) == 0) {
            ...
            //console->DevMsg("chaos: Found interface %s at %p in %s!\n", current->m_pName, result, filename);
            break;
        }
    }

    if (!result) {
        console->DevWarning("chaos: Failed to find interface with symbol %s in %s!\n", interfaceSymbol, filename);
        return nullptr;
    }
    return result;
}
