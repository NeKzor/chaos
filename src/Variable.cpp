...
{
    if (chaos.game->version & SourceGame_Portal2Engine) {
...
void Variable::DisableChange()
{
    if (this->ptr) {
        ...
        } /* else if (chaos.game->version & SourceGame_HalfLife2Engine) {
...
int Variable::RegisterAll()
{
    ...
    for (const auto& var : Variable::list) {
        if (var->version != SourceGame_Unknown && !(var->version & chaos.game->version)) {
            continue;
        }
        var->Register();
        ++result;
